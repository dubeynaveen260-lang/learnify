...
// CGPA Modal Functions

// Open CGPA Calculator in full screen modal
...
// Close CGPA Calculator modal
function closeCGPAModal() {
    const modal = document.getElementById('cgpaModal');
    if (modal) {
        modal.style.display = 'none';
        document.body.style.overflow = 'auto'; // Restore scrolling
        resetCGPACalculatorModal();
    }
}

// Generate semester input fields for modal
function generateSemesterInputsModal() {
    const numSemesters = parseInt(document.getElementById('numSemestersModal').value);
    const container = document.getElementById('semesterInputsContainerModal');
    ...
    for (let i = 1; i <= numSemesters; i++) {
        html += `
            <div class="semester-input-card">
                ...
                <div class="form-group">
                    <label>Number of Subjects:</label>
                    <input type="number" id="sem${i}_subjects_modal" min="1" max="10" value="5" 
                           onchange="generateSubjectInputsModal(${i})">
                </div>
                <div id="sem${i}_subjectsContainer_modal" class="subjects-container">
                    <!-- Subject inputs will appear here -->
                </div>
            </div>
        `;
    ...
    // Initialize subject inputs for each semester
    for (let i = 1; i <= numSemesters; i++) {
        generateSubjectInputsModal(i);
    }
}

// Generate subject input fields for a semester in modal
function generateSubjectInputsModal(semesterNum) {
    const numSubjects = parseInt(document.getElementById(`sem${semesterNum}_subjects_modal`).value);
    const container = document.getElementById(`sem${semesterNum}_subjectsContainer_modal`);
    ...
    for (let i = 1; i <= numSubjects; i++) {
        html += `
            <div class="subject-input-row">
                <input type="text" placeholder="Subject ${i}" id="sem${semesterNum}_sub${i}_name_modal" class="subject-name-input">
                <input type="number" placeholder="Credits" id="sem${semesterNum}_sub${i}_credits_modal" 
                       min="1" max="10" value="4" class="credits-input">
                <select id="sem${semesterNum}_sub${i}_grade_modal" class="grade-select">
                    <option value="">Select Grade</option>
                    <option value="10">O (Outstanding) - 10</option>
                    <option value="9">A+ (Excellent) - 9</option>
                    <option value="8">A (Very Good) - 8</option>
...
// Calculate CGPA in modal
function calculateCGPAModal() {
    const numSemesters = parseInt(document.getElementById('numSemestersModal').value);
    ...
    // Calculate for each semester
    for (let sem = 1; sem <= numSemesters; sem++) {
        const numSubjects = parseInt(document.getElementById(`sem${sem}_subjects_modal`).value);
        ...
        for (let sub = 1; sub <= numSubjects; sub++) {
            const credits = parseFloat(document.getElementById(`sem${sem}_sub${sub}_credits_modal`).value);
            const grade = parseFloat(document.getElementById(`sem${sem}_sub${sub}_grade_modal`).value);
    ...
    // Display results
    displayCGPAResultsModal(cgpa, percentage, semesterResults);
}

// Display CGPA calculation results in modal
function displayCGPAResultsModal(cgpa, percentage, semesterResults) {
    const resultsDiv = document.getElementById('cgpaResultsModal');
    const overallCGPAEl = document.getElementById('overallCGPAModal');
    const percentageEl = document.getElementById('equivalentPercentageModal');
    const semesterWiseEl = document.getElementById('semesterWiseSGPAModal');
...
// Reset CGPA calculator modal
function resetCGPACalculatorModal() {
    document.getElementById('numSemestersModal').value = '';
    document.getElementById('semesterInputsContainerModal').innerHTML = '';
    document.getElementById('cgpaResultsModal').style.display = 'none';
    showNotification('Calculator reset', 'info');
}

// Download CGPA result as text from modal
function downloadCGPAResultModal() {
    const cgpa = document.getElementById('overallCGPAModal').textContent;
    const percentage = document.getElementById('equivalentPercentageModal').textContent;
    const numSemesters = parseInt(document.getElementById('numSemestersModal').value);
